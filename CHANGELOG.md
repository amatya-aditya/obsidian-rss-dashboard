...
- Added `docs/test_feeds/`, an archive of real-world RSS/Atom/JSON Feed documents kept for manual QA and bug-hunting, separate from the synthetic fixtures under `test_files/unit/**/fixtures/`. First entry: the RSS Advisory Board's own RSS 2.0 spec-compliance sample feed, which uncovered both the "Invalid Date" display bug and the RFC 822 named-timezone parsing bug fixed above. See `docs/test_feeds/index.md`.
