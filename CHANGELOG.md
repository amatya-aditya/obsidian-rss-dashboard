...
- Added an **Import starred articles** entry point next to Import OPML that reads an exported `starred.json` (Inoreader / Google Reader API "Read later" format) and previews matching starred items grouped by source feed with per-article and per-feed selection before inserting the selected articles — with `starred` set and the export's read state preserved — into their feed. For feeds you already subscribe to, articles are inserted directly. For feeds you don't, the preview groups them under a "New feed" row with an editable target folder (matching the OPML importer's inline folder-edit control); on import, the feed is created in that folder, a single background fetch populates its metadata and current items, and the historical starred item is inserted and marked starred/read immediately, without waiting on that fetch. An off-by-default **Fetch full article content** toggle in the preview reuses the existing full-article-fetch pipeline to replace each selected article's content with its fetched full text after import; a per-article fetch failure doesn't block the rest of the import and is reported afterward in a results summary linking to the original article, with a suggestion to try the Obsidian web clipper browser extension as a manual fallback. [GH Issue #234](https://github.com/amatya-aditya/obsidian-rss-dashboard/issues/234)
...
- The new-feed folder-assignment control in the Import starred articles preview now leads with the same folder icon used for the OPML importer's folder rows, and the preview shows brief helper text explaining that new feeds land in an editable target folder, whenever the export contains at least one new feed. [GH Issue #234](https://github.com/amatya-aditya/obsidian-rss-dashboard/issues/234)
...
- Opening a starred-imported article in the reader whose full content has never been fetched, or whose last fetch attempt failed, now shows a cached-preview banner naming the `starred.json` import date/time, with a **Fetch now** action alongside the existing **Open in Browser** action, instead of silently auto-fetching on open. Clicking **Fetch now** reuses the same full-article-fetch pipeline as the import-time toggle; a successful fetch replaces the article's content and, when the article is starred or saved, persists it, while a failed fetch marks the article so the banner can distinguish "failed" from "never attempted" on the next open. Reader behavior for articles that didn't come from a starred import is unaffected. [GH Issue #234](https://github.com/amatya-aditya/obsidian-rss-dashboard/issues/234)
