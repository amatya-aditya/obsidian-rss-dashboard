...
- Shard storage v2 now upgrades `user-state.json` to version 3 and uses session-scoped, hydration-gated 90-day cleanup for article state absent from validated feed shards, while preserving state for missing, corrupt, or unhydrated shards and safely aging legacy bare-GUID state. [GH Issue #315](https://github.com/amatya-aditya/obsidian-rss-dashboard/issues/315)
- The Storage status line in Settings → Storage now reports the resolved path of the `data.json` metadata is actually written to, alongside the existing shard folder. When metadata is stored in a vault folder, the `data.json` inside the plugin folder is reduced to a small pointer and the real file lives in the vault — which made "where did my data go" hard to answer after switching storage modes, particularly when reverting to Legacy JSON.
- Settings → Storage now reports a `user-state.json` left behind by a previous Shard storage v2 setup, naming its path. Only v2 reads that file, so after reverting to Legacy JSON or stepping down to Shard storage v1 it sits in the metadata folder looking current while the plugin has stopped consulting it. It is deliberately not deleted — after a revert it is the only standalone copy of read, starred, tagged, and saved state — so it is surfaced for you to remove by hand once you are sure you no longer need it. [GH Issue #284](https://github.com/amatya-aditya/obsidian-rss-dashboard/issues/284)
- Legacy JSON and Shard storage v1 are now formally deprecated, and the storage prompt says so: it names 3.0 as the release that turns those modes read-only and spells out what stops working (feed refresh, and recording read state, stars, tags, and saved articles). The old permanent **Never show again** dismissal is replaced by **Skip this version**, which defers the prompt to the next minor release rather than silencing it forever, and is withdrawn after three deferrals so the prompt stays visible — still deferrable to the next load via **Remind me later**, but no longer silenceable ahead of a cutoff users would otherwise meet as breakage. Anyone who previously chose **Never show again** is deliberately prompted once more, since that choice was consent to stop advertising an optional upgrade, not consent to a deprecation. The prompt now checks when the RSS Dashboard view opens rather than whenever Obsidian finishes loading, so it no longer appears for a session where the dashboard is never opened. See [ADR 0006](docs/adr/0006-deprecate-legacy-json-and-shard-storage-v1.md).
...
- Renamed **Import starred articles from Inoreader** to **Import starred articles** across the command palette, Settings tab, Feed Manager button, and modal title/description, and renamed the default new-feed folder for new imports from **Inoreader starred imports** to **Starred imports**; existing folders, feeds, settings, and command IDs are unchanged. The importer now also accepts a FreshRSS `starred.json` export: it resolves a portable source URL from `origin.htmlUrl` when `origin.streamId` is an instance-local numeric ID (e.g. `feed/6`) rather than a URL, and reads article HTML from `content.content` when `summary.content` is absent. See [docs/starred-import-compatibility.md](docs/starred-import-compatibility.md) for the parser contract and the narrow, per-service scope of what's actually confirmed compatible, and [ADR 0003](docs/adr/0003-generalize-starred-import-to-google-reader-compatible-naming.md) for the naming decision. [GH Issue #330](https://github.com/amatya-aditya/obsidian-rss-dashboard/issues/330) [GH Issue #337](https://github.com/amatya-aditya/obsidian-rss-dashboard/issues/337)
...
- Fixed the reader header and the dashboard's card/list/feed view date badges showing the literal text "Invalid Date"/"Invalid date" for articles with no declared publish date, instead of falling back to the article's first-seen date the way retention and sorting already do. They now show `First seen: <date>` when falling back, and "Unknown date" in the rare case neither a publish date nor a first-seen date is available.
- Fixed `<pubDate>` values using an obsolete named US timezone abbreviation (e.g. `CST`, `PST`, `EST`) — as found in very old RSS feeds — sometimes parsing as an invalid date. Support for these abbreviations in `Date.parse()` is implementation-defined rather than standardized, so behavior can vary across the Chromium/V8 build Obsidian bundles; the abbreviation is now normalized to an explicit UTC offset before parsing so the result no longer depends on the runtime.
- Fixed the reader header and dashboard date badges showing a first-seen date even when **Use first-seen date for undated items** is turned off. Display was falling back to the first-seen date unconditionally; it now respects the setting the same way sorting and retention already do, showing "Unknown date" instead when the setting is off and no publish date exists.
- Fixed the same "Invalid Date" display bug in the video player, the podcast episode list, and the reader's video-item view for articles with no declared publish date; they now fall back to the first-seen date (when the setting above is on) the same way the reader header and dashboard date badges already do. The podcast episode list keeps omitting its date row entirely when there's nothing to show, rather than adding a new "Unknown date" placeholder it never had before.
- Fixed the article context menu's **Open in browser** silently opening a blank tab, and **Copy article URL** copying an empty string while still claiming success, for articles with no `<link>` (permitted by the RSS 2.0 spec when the item has a non-permalink `<guid>` instead). Both menu items are now omitted entirely when the article has no link, matching how **Copy feed URL** is already omitted when a feed has no URL.
- Fixed reverting from a shard storage mode back to Legacy JSON appearing to succeed and then showing an empty dashboard on the next launch. The revert wrote the full reverted settings straight into the plugin folder’s `data.json`, bypassing the configured metadata location and destroying the bootstrap pointer that normally lives there. Because those settings still recorded the vault metadata location, the next launch followed it back to the stale pre-revert copy and loaded that instead — so the dashboard came up in the old shard mode, with feeds whose articles lived in shard files that the revert had just deleted. As a side effect of loading that stale copy, the storage deprecation prompt also stopped appearing after a revert.
- Fixed plugin settings being replaced with defaults, and then saved over the existing configuration, when metadata is stored in a vault folder and that folder’s `data.json` cannot be read. An unreadable or missing file is now treated the same way as a first run — nothing is written back — and a notice reports it instead of the failure passing silently.
...
- Fixed saved-note frontmatter templates interpolating article titles, authors, feed titles, links, guids, and media URLs straight into double-quoted YAML scalars, so a title containing a `"` produced frontmatter that failed to parse and a crafted title could close the scalar and inject extra frontmatter keys; the same escaping now applies to both the article-saver and web-viewer save paths. [GH Issue #286](https://github.com/amatya-aditya/obsidian-rss-dashboard/issues/286)
- Fixed Shard storage v2's `user-state.json` write path silently discarding article state: it was rebuilt from scratch from in-memory feeds on every save, so a feed that failed to hydrate, hadn't yet synced its shard to this device, or had items pruned by retention lost the read/starred/tagged/saved/playback state for every affected article on the next write — and under sync, the deletion then propagated to other devices. State is now merged into what's already on disk and only updated for items actually loaded, so a feed loses its state only when it is genuinely removed from settings. Also fixed state being keyed by bare guid, so two feeds sharing a guid overwrote each other's state (now keyed by `feedId:guid`, migrating an existing file in place), and fixed a cleared "read" flag being unrepresentable, so marking an article unread couldn't survive a sync (an explicit `false` is now persisted and retained). A `user-state.json` that exists but can't be read is never overwritten; a notice explains that changes won't be saved until it is fixed or removed, and a red alert stays at the top of the dashboard's status strip (even if that strip is turned off, and even when it is collapsed) until the file is readable again. [GH Issue #278](https://github.com/amatya-aditya/obsidian-rss-dashboard/issues/278)
- Fixed deleting a feed or folder leaving its shard file behind on disk under Shard storage (v1 and v2) when the storage folder is a hidden folder such as the default `.rss-dashboard-data/feeds`. Obsidian does not index hidden folders, so the plugin could not find the shard to remove it and skipped the removal without any message; the same defect left the old folder's shards behind after changing the storage folder. Folder deletion now removes feeds in nested subfolders through the same persistence path. Shards are now removed by path when Obsidian has no indexed file for them. Shard files already orphaned by earlier deletions are not cleaned up by this fix. [GH Issue #316](https://github.com/amatya-aditya/obsidian-rss-dashboard/issues/316)
- Fixed toggling the sidebar collapse button rebuilding the entire dashboard — including the article list and content area — instead of just the sidebar. Collapsing or expanding is now a presentational change that only updates the sidebar's visibility, width, and resize handle. The sidebar's **Collapse/Expand all folders** action had the same problem (a full dashboard re-render on every click) and now refreshes only the sidebar.

### Development and compliance

- Hardened the GitHub release workflow by separating read-only build validation from privileged publication and provenance, pinning workflow Actions to reviewed commits, and attesting `manifest.json` alongside the plugin bundle. Also fixed the test.yml from 'on: push: branches: [main, develop]' to 'on: push: branches: [master, dev]' which matches our existing repo structure.
- Added `docs/development/test-feeds/`, a running reference list of real-world RSS/Atom/JSON Feed documents (some saved locally, some linked live) kept for manual QA and bug-hunting, separate from the synthetic fixtures under `test_files/unit/**/fixtures/`. Entries so far: the RSS Advisory Board's RSS 2.0 spec-compliance sample and an RSS 0.92 test feed (`rbren/rss-parser`), which together uncovered the "Invalid Date" display bug, the RFC 822 named-timezone parsing bug, and the first-seen-fallback-ignoring-its-own-setting bug fixed above; a minimal spec-clean RSS feed (Brave's fuzzer corpus) as a parsing baseline; and reference links to github.blog/feed (general QA gold standard) and the Terence Tao / Math StackExchange feeds used historically for MathJax rendering verification. See `docs/development/test-feeds/index.md`.
