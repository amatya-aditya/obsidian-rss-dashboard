...
- Added a What's New popup that opens once after an update, summarizing the current version's Features, with a link to the full changelog on GitHub. It never appears on a fresh install or for a release with no Features, and shows only once per version. The About settings tab gains a **What's new** link that reopens the same summary anytime, next to the version number. [GH Issue #288](https://github.com/amatya-aditya/obsidian-rss-dashboard/issues/288)
