...
- Fixed deleting a feed leaving its shard file behind on disk under Shard storage (v1 and v2) when the storage folder is a hidden folder such as the default `.rss-dashboard-data/feeds`. Obsidian does not index hidden folders, so the plugin could not find the shard to remove it and skipped the removal without any message; the same defect left the old folder's shards behind after changing the storage folder. Shards are now removed by path when Obsidian has no indexed file for them. Shard files already orphaned by earlier deletions are not cleaned up by this fix. [GH Issue #316](https://github.com/amatya-aditya/obsidian-rss-dashboard/issues/316)
