...
- Added an **Import starred articles** entry point next to Import OPML that reads an exported `starred.json` (Inoreader / Google Reader API "Read later" format) and, for feeds you already subscribe to, previews matching starred items grouped by source feed with per-article and per-feed selection before inserting the selected articles — with `starred` set and the export's read state preserved — into their existing feed. Items whose source feed isn't already subscribed to are not shown; auto-creating those feeds, surfacing unimportable entries, label-to-tag mapping, re-import dedup, and full-content fetch are tracked as follow-on work. [GH Issue #234](https://github.com/amatya-aditya/obsidian-rss-dashboard/issues/234)
