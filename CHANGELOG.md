...
- Added a **Sync now** action to the FreshRSS settings tab that links or creates local feeds from the connected account's subscriptions, imports a bounded set of recent articles through the existing content pipeline, and reports one summary notice. Linking only matches a subscription to a local feed with the same canonical feed URL and never guesses when more than one matches; a previously linked feed keeps its local folder, retention, templates, tags, refresh settings, and saved-note behavior; and the sync never deletes a local feed or article because it went missing from a FreshRSS response.
...
- Made FreshRSS read/unread state offline-first and bidirectional. Every read/unread action (reader, dashboard, context menu, automatic-read-on-open, mark page as read, sidebar selection, folder, and all-feed actions) now routes through one mutation boundary: for a FreshRSS-linked article, the read/unread choice is durably queued in the FreshRSS sidecar under the shared data-sync lease before it is applied locally, so it survives offline use and app restarts, and a queuing failure leaves the local state unchanged with a clear error instead of silently losing the change. A newer choice replaces an older queued one instead of stacking history. Each **Sync now** cycle now obtains a fresh token and flushes queued read changes to FreshRSS before pulling remote read state, and a queued local change always takes priority over a conflicting value pulled from the server until FreshRSS acknowledges it with an exact `OK` response. Local-only (non-FreshRSS) articles are completely unaffected.
- Extended the same offline-first mutation boundary to FreshRSS starred state. Every star/unstar action (reader, inline reader, dashboard, context menu, and all other existing entry points) now routes through the shared article-facet mutation boundary alongside read/unread: for a FreshRSS-linked article, the star/unstar choice is durably queued in the FreshRSS sidecar before it is applied locally, a newer choice replaces an older queued one, and each **Sync now** cycle flushes queued starred changes (using the same fresh token as read changes) before pulling and reconciling remote starred state, never overriding a still-queued local choice. Local starred-retention protection and the automatic local "Favorite" tag continue to work exactly as before and independently of FreshRSS; starring an article never creates an unrelated FreshRSS label change. Local-only (non-FreshRSS) articles star and unstar exactly as before, with no sidecar or network activity.
- Extended offline-first FreshRSS sync to mapped labels. Each **Sync now** cycle now discovers FreshRSS's tag/label list and builds a local mapping keyed by a trimmed, locale-independent lower-cased label name to its exact opaque remote tag reference; the dedicated read and starred system streams and FreshRSS category/folder entries are never exposed as dashboard tags, and two remote labels that normalize to the same name are excluded from mapping rather than guessed. Adding or removing a KNOWN mapped label on a FreshRSS-linked article's local tags now queues one durable `label:<name>` mutation in the sidecar before the tag change is applied, using the same sidecar-before-commit, coalescing, and stale-acknowledgment-safe guarantees as read/starred; a still-queued local label choice is never overridden by a conflicting pulled value, and remote membership changes never touch a local tag's chosen display spelling or color. Unmapped local tags, the automatic "Favorite"/"Saved" tags, folder auto-tags, and other local-only tags continue to create no sidecar record and no FreshRSS request. Version 1 never creates, renames, or deletes a FreshRSS label — a local tag without a known remote mapping simply stays local.
- Hardened the FreshRSS **Sync now** cycle for large accounts and unreliable connections. Every request (login, reads, and mutation dispatch) now gets up to three in-cycle attempts with 1s/2s/4s bounded-jitter delays on a network failure, timeout, or HTTP 408/429/5xx; a cycle that still ends with an exhausted transient failure schedules the next automatic attempt with exponential backoff starting at 5 minutes and capped at 6 hours, reset by the next fully successful cycle. A mid-cycle HTTP 401/403 now triggers exactly one fresh login before the affected request is retried once, matching the same "one fresh retry, then pause" rule already used for the initial cycle login. Facet mutations rejected with a non-retryable HTTP 400/404/422 (an unknown binding or an invalid label operation) now enter a durable terminal repair state instead of being retried automatically forever — the desired state and rejection reason stay on file for an explicit user retry or cancel, and the sync summary notice now calls out when a cycle completed with outstanding terminal rejections needing attention. Bounded item-ID enumeration (used for read/starred/label reconciliation) now stops at a 25,000-ID-per-stream budget and reports the cycle as partial rather than ever treating a capped or interrupted stream as authoritative for clearing local state.
- Added opt-in automatic FreshRSS synchronization, off by default, with a dedicated interval (defaulting to 15 minutes) fully independent of the ordinary feed refresh interval. When enabled, a startup attempt runs once settings, content shards, `user-state.json`, and `freshrss-state.json` have finished hydrating, and the dedicated timer then keeps syncing on that interval. Startup, the timer, **Sync now**, and an explicit retry all enter through the same coordinator used before, sharing the same data-sync lease with ordinary refresh; any trigger arriving while a cycle is already running coalesces into at most one trailing cycle rather than starting a second one. A successful automatic cycle rearms from the configured interval, counting from completion; a cycle that exhausts in-cycle transient retries instead rearms from the existing backoff deadline. A capability, storage, credential, connection-test, authentication, or persistence block never spins the timer or a notification loop — it simply waits for repair, a configuration change, a successful connection test, or an explicit retry. Automatic success stays silent; an automatic cycle only shows a notice when its outcome differs from the last one reported, or when it now needs user action. Plugin unload stops the dedicated timer and guards a late in-flight automatic cycle from updating connection status, showing a notice, or rearming after unload.
- Added a bounded, per-feed **Fetch more history** action to the FreshRSS settings tab: a dropdown lists only active-scope FreshRSS-linked feeds whose imported history is capped or incomplete (excluding local-only feeds and feeds already fully imported), and each invocation extends the selected feed's history by up to 25,000 additional remote item IDs -- the same default bootstrap budget already documented for other FreshRSS streams -- through the same coordinator, session, shared data-sync lease, paging, retry, retention, and persistence machinery as **Sync now**, rather than a parallel invocation path. Progress resumes from a persisted per-feed checkpoint cursor, so repeated invocations accumulate deeper history without duplicating local articles or re-walking already-imported pages; a capped or interrupted invocation stays partial and never clears existing read, starred, or mapped-label state, and any pending facet mutation for an older article discovered by this action stays authoritative. The completion notice names the selected feed and reports a bounded article count only -- never a remote ID.
- Added a dedicated FreshRSS subscription OPML export, separate from the existing generic OPML exporter, available from the FreshRSS settings tab (**Export FreshRSS OPML** / copy-to-clipboard) and the `Export FreshRSS subscription OPML` command, and independent of any FreshRSS connection. Every feed outline carries `text`, `title`, `type="rss"`, and the exact `xmlUrl`, with `htmlUrl` included only when RSS Dashboard already holds a reliable site URL for that feed. Uncategorized feeds sit directly under the OPML body; a nested local folder path (e.g. `Tech/AI/Blogs`) projects to one flat FreshRSS category by escaping `%` to `%25` and `/` to `%2F` within each segment before joining segments with `/`. Feeds sharing an identical URL collapse deterministically, keeping the first feed in dashboard order and reporting every collapse as a notice and console warning. The artifact is a subscriptions-only OPML 2.0 document — no credentials, sessions, tokens, plugin settings, FreshRSS sidecar data, article bodies, read/starred state, dashboard tags, FreshRSS labels, saved-note state, or `frss:*` extensions are ever included, and all UI copy calls it a FreshRSS subscription export rather than an article-state backup.

### Fixes

- Fixed sidebar **Delete tag** so a FreshRSS-mapped label is no longer silently re-added by the next sync. Deleting a tag now routes through the same FreshRSS article-facet mutation boundary as every other tag change: for each affected FreshRSS-linked article, the mapped-label removal is durably queued -- atomically, under one shared write, across every affected article at once -- before the tag is actually removed anywhere; if that queuing fails (e.g. FreshRSS sync is busy), nothing is changed and the tag stays defined and stays on every article, rather than deleting it locally while leaving FreshRSS unaware. Unmapped and local-only tags are unaffected.
- Fixed the per-article tag-picker's trash-icon **Delete tag** action (Dashboard and Reader view) so it goes through the same FreshRSS article-facet mutation boundary as the sidebar's **Delete tag** action, closing the last of three UI entry points that shared this bug. A FreshRSS-mapped label's removal is now durably queued for every affected article before the tag is removed anywhere; a queuing failure leaves the tag defined and untouched on every article instead of deleting it locally while FreshRSS stays unaware. Unmapped and local-only tags are unaffected.
- Fixed the FreshRSS sync-outcome notice (manual and automatic) so it no longer tells users a rejected change can be "retried or cancelled from FreshRSS settings" -- no such per-mutation control exists in the UI. The notice now states the actual current behavior: a terminally rejected change stays on hold and is not retried automatically, and reviewing or retrying it from settings isn't available yet. See `docs/development/freshrss-rollout-validation.md` for the tracked follow-up to build that control.
...
- Added a pinned FreshRSS Docker read-contract harness (`docker/freshrss-contract/`, `npm run contract:freshrss`, `npm run test:freshrss-fixtures`, and the `FreshRSS Docker contract` CI workflow) that proves the FreshRSS read path against one official, digest-pinned FreshRSS image (`ghcr.io/freshrss/freshrss:1.29.1@sha256:ab6b363102ccdbc39f6a62db926f567c61a5289bf25ba460f1c34423d8cc1a4d`), separately from the ordinary unit suite, which continues to require neither Docker nor network access. See `docs/development/freshrss-docker-contract.md` for the exact status and what a first real run still needs to confirm.
- Extended the FreshRSS Docker contract harness with state-mutation and OPML round-trip scenarios: two more deterministic fixtures (`feed-d`, seeded at boot, for read/starred/mapped-label mutation contrast plus a deeper 3-page item-ID pagination; `feed-e`, seeded only mid-run through the OPML scenario itself), verification of the exact FreshRSS modification-token and `OK`-acknowledgment behavior for read, starred, and mapped-label-membership changes (each proven not to leak onto an untouched control article), restart-persistence coverage extended to all three mutated facets, and an OPML round-trip scenario that exports RSS Dashboard subscriptions through the real production `generateFreshRssSubscriptionOpml` (including a deliberate duplicate-feed-URL case), imports that OPML into the live container, compares FreshRSS's own re-exported OPML for URL/title/flat-category/duplicate-collapse agreement, and confirms a pre-existing baseline subscription is untouched by the import — scoped exactly as specified, with no assertion about credentials, article bodies, read/starred state, labels, sidecar data, or FreshRSS archival state. A "previous stable image" advisory CI job is scaffolded as a non-blocking, intentionally unimplemented stub (tracked via a `TODO` in the workflow file) rather than a casual second pinned image. **This harness has now had a successful live run** (2026-09-09, against the exact pinned image/digest above) that also caught and fixed a real bug in the harness's own original "uncategorized feed" assumption, and surfaced three further findings worth follow-up tickets against `src/services/freshrss-sync-client.ts` and `freshrss-sync-coordinator.ts` (FreshRSS never reports an empty `categories` array; a `tag/list` label entry has no `label` field on this pinned build) — see `docs/development/freshrss-docker-contract.md` for the full, itemized findings and what is still only proven by local runs rather than CI.
- Closed out the FreshRSS portable-state-client workstream with rollout validation and compatibility documentation: a second, independent live Docker contract run (2026-09-09, `overallPassed: true`, same 21 scenarios and same three findings as the first run) is recorded alongside a finite tested compatibility matrix in `docs/development/freshrss-docker-contract.md`; a new `docs/development/freshrss-rollout-validation.md` maps every rollout checklist item to either its automated test coverage or an explicit manual-verification checklist for a human with real Obsidian, and flags one real gap found in passing (the sync-outcome notice text references a per-mutation terminal retry/cancel control in FreshRSS settings that does not currently exist in the UI); a new user-facing `docs/freshrss-guide.md` (linked from the README) documents setup, connection scope, synchronized-versus-local state, pending facet mutations, repair, history bounds, and OPML limits; `docs/SECURITY.md` now itemizes exactly which persisted files and artifacts are proven never to contain FreshRSS credential/auth/session/token material and cites the tests; and a stale "issue #224 remained open" planning-time note in `docs/plans/226-freshrss-test-architecture-docker-contract.md`, plus duplicate/inconsistent "FreshRSS connection scope" and pending-mutation glossary entries in `CONTEXT.md`, are corrected without changing any settled decision.
...
- RSS Dashboard now requires Obsidian 1.11.4 or later (raised from 1.8.7): FreshRSS's optional SecretStorage capability-gate code statically references an API that first shipped in 1.11.4, and `eslint-plugin-obsidianmd`'s recommended config forbids suppressing that check, so the declared floor must cover it even though the capability itself remains off unless a user opts into FreshRSS.
