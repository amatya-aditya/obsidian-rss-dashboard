...
- Unread articles older than a feed's auto-delete cutoff are now expired by default, fixing unexpected storage accumulation where unread articles were kept indefinitely regardless of the configured retention window. Four configurable protection toggles (`protectStarred`, `protectSaved`, `protectTagged`, `protectUnread`) are now stored in settings and used by all retention limit operations — `applyFeedRetentionLimits`, the feed-parser carry-forward, fresh-item ingest, and `applyFeedLimitsToAllFeeds` — so each article state can be independently opted out of deletion. Existing users keep their starred and saved protections (`true` by default). [GH Issue #213](https://github.com/amatya-aditya/obsidian-rss-dashboard/issues/213)
