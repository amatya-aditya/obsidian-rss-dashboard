			...
			.addDropdown((dropdown) =>
				dropdown
					...
					.onChange(async (value: string) => {
						this.plugin.settings.viewStyle = value as
							| "list"
							| "card";
		...
		new Setting(containerEl)
			...
			.addDropdown((dropdown) =>
				dropdown
					...
					.onChange(async (value: string) => {
						this.plugin.settings.viewLocation =
							value as ViewLocation;
						await this.plugin.saveSettings();
					}),
			);
		...
		new Setting(containerEl)
			...
			.addDropdown((dropdown) =>
				dropdown
					...
					.onChange(async (value: string) => {
						this.plugin.settings.readerViewLocation =
							value as ViewLocation;
						await this.plugin.saveSettings();
					}),
			);
				...
				.addToggle((toggle) =>
					toggle.setValue(!isHidden).onChange(async (value) => {
						...
						if (value) {
							...
							if (
								!this.plugin.settings.display.hiddenFilters.includes(
									filter.key,
								)
							) {
								this.plugin.settings.display.hiddenFilters.push(
									filter.key,
								);
							}
				...
				dropdown
					...
					.onChange(async (value) => {
						this.plugin.settings.media.podcastTheme =
							value as PodcastTheme;
						await this.plugin.saveSettings();
						const readerView =
							await this.plugin.getActiveReaderView();
						if (readerView) {
	...
	private createArticleSavingSettings(containerEl: HTMLElement): void {
		...
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle
					.setValue(
						this.plugin.settings.articleSaving.saveFullContent,
					)
					.onChange(async (value) => {
						this.plugin.settings.articleSaving.saveFullContent =
							value;
		...
		new Setting(containerEl)
			...
			.addSlider((slider) => {
				slider
					.setLimits(5, 30, 1)
					.setValue(
						this.plugin.settings.articleSaving.fetchTimeout || 10,
					)
					.setDynamicTooltip()
					.onChange(async (value) => {
		...
		saveAsTemplateBtn.onclick = async () => {
			...
			if (name) {
				...
				this.plugin.settings.articleSaving.savedTemplates.push(
					newTemplate,
				);
				await this.plugin.saveSettings();
				...
				new Setting(templatesContainer)
					...
					.addButton((button) =>
						button
							...
							.setTooltip(
								"Update this template with current editor content",
							)
							.onClick(async () => {
								this.plugin.settings.articleSaving.savedTemplates[
									index
	...
	private createTagsSettings(containerEl: HTMLElement): void {
		...
		new Setting(newTagContainer).addButton((button) =>
			button.setButtonText("Add tag").onClick(async () => {
				...
				const colorPicker = tagColorSetting
					.components[0] as unknown as { getValue: () => string };
