				...
				feed.domain.forEach((domain: string) => {
					...
					if (feed.subdomain && feed.subdomain.length > 0) {
						feed.subdomain.forEach((subdomain: string) => {
							...
							const subdomainNode = domainNode[
								subdomain
							] as CategoryNode;
	...
	private renderCategoryNode(
		...
		keys.forEach((key) => {
			...
			const isSelected = this.filters.selectedPaths.some(
				(p: CategoryPath) =>
					this.categoryPathToString(p) === currentPathStr,
			);
				...
				if (checkbox.checked) {
					if (
						!this.filters.selectedPaths.some(
							(p: CategoryPath) =>
								this.categoryPathToString(p) === pathStr,
						)
					) {
						this.filters.selectedPaths.push(
							this.stringToCategoryPath(pathStr),
						);
					}
				} else {
					this.filters.selectedPaths =
						this.filters.selectedPaths.filter(
							(p: CategoryPath) =>
								this.categoryPathToString(p) !== pathStr,
						);
				}
			...
			if (hasChildren) {
				...
				expandIcon.addEventListener("click", () => {
					const isCollapsed =
						childrenContainer.hasClass("rss-collapsed");
					if (isCollapsed) {
