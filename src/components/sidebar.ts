...
export class Sidebar {
		...
		folderHeader.addEventListener("contextmenu", (e) => {
			...
			menu.addItem((item: MenuItem) => {
				item.setTitle("Mark all as read")
					...
					.onClick(() => {
						const allPaths =
							this.getAllDescendantFolderPaths(fullPath);
			...
			if (e.dataTransfer) {
				...
				if (feedUrl) {
					...
					if (feed && feed.folder !== fullPath) {
						if (feed.folder) {
							const oldFolder = this.findFolderByPath(
								feed.folder,
							);
							if (oldFolder) oldFolder.modifiedAt = Date.now();
						}
		...
		addButton.addEventListener("click", () => {
			void (async () => {
				...
				if (/^UC[\w-]{22}$/.test(channel)) {
					...
					const result =
						await this.extractChannelIdAndNameFromYouTubePage(
	...
	private async addSubfolderByPath(
		parentPath: string,
		...
		if (
			parent &&
			!parent.subfolders.some((f) => f.name === subfolderName)
		) {
			...
			return folders.filter((folder: Folder) => {
				if (folder.name === parts[depth]) {
					if (depth === parts.length - 1) {
						...
						folder.subfolders = removeRecursive(
							folder.subfolders,
							depth + 1,
						);
						return true;
						...
						.onClick(() => {
							if (feed.folder !== folderPath) {
								if (feed.folder) {
									...
									if (oldFolder)
										oldFolder.modifiedAt = Date.now();
								}
								feed.folder = folderPath;
								const newFolder =
									this.findFolderByPath(folderPath);
								if (newFolder)
									newFolder.modifiedAt = Date.now();

								void this.plugin.saveSettings().then(() => {
									this.render();
									new Notice(
										`Moved "${feed.title}" to "${folderPath}"`,
									);
								});
							}
						});
					...
					this.showFolderNameModal({
						...
						onSubmit: (folderName) => {
							void (async () => {
								...
								const newFolder =
									this.findFolderByPath(folderName);
								if (newFolder)
									newFolder.modifiedAt = Date.now();
								await this.plugin.saveSettings();
								this.render();
	...
	private toggleAllFolders(): void {
		...
		if (allCollapsed) {
			...
			this.callbacks.onBatchToggleFolders?.(
				foldersToCollapse,
				foldersToExpand,
			);
