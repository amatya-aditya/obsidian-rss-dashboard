...
export class DiscoverView extends ItemView {
	...
	private filterFeeds(): void {
		this.filteredFeeds = this.feeds.filter((feed) => {
			...
			if (this.filters.selectedPaths.length > 0) {
				const hasMatchingPath = this.filters.selectedPaths.some(
					(selectedPath: CategoryPath) => {
						...
						if (
							selectedPath.area &&
							!feed.area.includes(selectedPath.area)
						) {
							return false;
						}
			...
			if (this.filters.selectedTags.length > 0) {
				const hasMatchingTag = this.filters.selectedTags.some(
					(tag: string) => feed.tags.includes(tag),
				);
				if (!hasMatchingTag) {
					return false;
				}
				...
				checkbox.addEventListener("change", () => {
					if (checkbox.checked) {
						...
						this.filters.selectedTags =
							this.filters.selectedTags.filter(
								(t: string) => t !== tag,
							);
					}
					this.currentPage = 1;
	...
	private renderContent(container: HTMLElement): void {
		...
		const hamburgerMenu = topSection.createDiv({
	...
	private populateFilterDropdown(
		...
		options.forEach((option) => {
			...
			count.textContent = this.getOptionCount(
				filterType,
				option,
			).toString();
	...
	private getOptionCount(filterType: string, option: string): number {
		switch (filterType) {
			...
			case "domain":
				return this.feeds.filter((f) => f.domain.includes(option))
					.length;
			case "subdomain":
				return this.feeds.filter((f) => f.subdomain.includes(option))
					.length;
			case "area":
				return this.feeds.filter((f) => f.area.includes(option)).length;
			case "topic":
				return this.feeds.filter((f) => f.topic.includes(option))
					.length;
			default:
				return 0;
		}
		...
		switch (filterType) {
			case "type":
				if (selected) {
					...
					this.filters.selectedTypes =
						this.filters.selectedTypes.filter((t) => t !== option);
				}
				break;
			case "tag":
				if (selected) {
					...
					this.filters.selectedTags =
						this.filters.selectedTags.filter((t) => t !== option);
				}
				break;
			case "domain":
			case "subdomain":
			...
			case "topic":
				if (selected) {
					...
					this.filters.selectedPaths =
						this.filters.selectedPaths.filter((p) => {
							switch (filterType) {
								case "domain":
