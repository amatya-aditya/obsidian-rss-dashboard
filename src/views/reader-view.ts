...
export class ReaderView extends ItemView {
	...
	private getCustomTemplateForArticle(item: FeedItem): string | undefined {
		...
		if (feed?.customTemplate) {
			const articleSaving: ArticleSavingSettings =
			...
			menuItem
				...
				.onClick(async () => {
					...
					const customTemplate =
						this.getCustomTemplateForArticle(item);
		...
		if (item.saved) {
			...
			if (!fileExists) {
				...
				if (item.feedUrl) {
					...
					if (feed) {
						const originalItem = feed.items.find(
							(i) => i.guid === item.guid,
						);
						if (originalItem) {
	...
	private async displayPodcast(item: FeedItem): Promise<void> {
		...
		if (item.feedUrl) {
			...
			if (feed) {
				fullFeedEpisodes = feed.items.filter(
					(i) => i.mediaType === "podcast",
				);
			}
		}
	...
	private renderArticle(item: FeedItem, fullContent?: string): void {
		...
		const htmlString = ensureUtf8Meta(
		...
		function appendNodes(
			...
			nodes.forEach((node) => {
				if (node.nodeType === Node.TEXT_NODE) {
					...
					const isIconElement =
						element.tagName === "I" &&
						element.classList.contains("icon-class");
					if (!isIconElement) {
						const tag =
