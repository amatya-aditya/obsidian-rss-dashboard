...
export class RssDashboardView extends ItemView {
	...
	private getFilteredArticles(): FeedItem[] {
		...
		if (this.currentFeed) {
			const limit =
				this.currentFeed.maxItemsLimit || this.settings.maxItems;
			...
			if (specialFolders.includes(this.currentFolder)) {
				...
				const allFolders = this.getAllDescendantFolders(
					this.currentFolder,
				);
				for (const feed of this.settings.feeds) {
		...
		if (
			...
			articles = articles.filter((item) => {
				...
				if (this.filterLogic === "AND") {
					...
					if (this.activeStatusFilters.has("unread") && isRead)
						return false;
					if (this.activeStatusFilters.has("read") && !isRead)
						return false;
					if (this.activeStatusFilters.has("saved") && !isSaved)
						return false;
					...
					if (this.activeTagFilters.size > 0) {
						...
						const tagMatch = Array.from(this.activeTagFilters).some(
							(tagName) => itemTagNames.includes(tagName),
						);
					...
					if (this.activeTagFilters.size > 0) {
						if (item.tags && item.tags.length > 0) {
							const itemTagNames = item.tags.map((t) => t.name);
							if (
								Array.from(this.activeTagFilters).some(
									(tagName) => itemTagNames.includes(tagName),
								)
							) {
								return true;
		...
		if (
			...
			articles = articles.filter(
				(a) => new Date(a.pubDate).getTime() > maxAge,
			);
		}
	...
	private findFolderByPath(path: string): Folder | null {
		...
		for (let i = 1; i < parts.length && current; i++) {
			current = (current.subfolders || []).find(
				(f) => f.name === parts[i],
			);
		}
		return current || null;
		...
		if (this.currentFeed) {
			...
			const feedsWithTag = this.settings.feeds.filter((feed) =>
				feed.items.some(
					(item) =>
						item.tags &&
						item.tags.some((tag) => tag.name === this.currentTag),
				),
			);
	...
	public updateArticleSaveButton(articleGuid: string): void {
		...
		if (articleEl) {
			const saveButton = articleEl.querySelector(
			...
			for (const feed of this.settings.feeds) {
				articles = articles.concat(
					feed.items.filter(
						(item) =>
							item.tags &&
							item.tags.some((t) => t.name === this.currentTag),
					),
				);
			}
		} else if (this.currentFolder) {
	...
	private async findSavedArticleFile(
		article: FeedItem,
	): Promise<TFile | null> {
		if (!article.saved) {
			return null;
	...
	private async handleOpenSavedArticle(article: FeedItem): Promise<void> {
		...
		try {
			...
			if (savedFile) {
				...
				loadingNotice.hide();
				new Notice(
					"Saved article file not found. Article status updated.",
				);
			}
		} catch (error) {
			loadingNotice.hide();
