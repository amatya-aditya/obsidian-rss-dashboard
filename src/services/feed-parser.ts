...
async function resolveApplePodcastUrl(
	...
	if (!podcastId) {
		throw new Error(
			"Invalid Apple Podcasts URL: could not extract podcast ID",
		);
	}
...
async function discoverFeedUrl(baseUrl: string): Promise<string | null> {
	try {
		...
		if (baseUrl.includes("feeds.feedburner.com")) {
			const feedNameMatch = baseUrl.match(
...
export async function fetchFeedXml(url: string): Promise<string> {
	...
	async function tryFetch(targetUrl: string): Promise<string> {
		if (targetUrl.includes("feeds.feedburner.com")) {
			...
			const feedNameMatch = httpsUrl.match(
				/feeds\.feedburner\.com\/([^/?]+)/,
			);
			if (feedNameMatch) {
				const feedName = feedNameMatch[1];
			...
			try {
				...
				if (!isAndroid) {
					try {
						...
						if (
							codetabsResponse.text &&
							isValidFeed(codetabsResponse.text)
						) {
						...
						console.warn(
							"[RSS dashboard] codetabs proxy failed",
							e,
						);
					}
					...
					try {
						...
						console.warn(
							"[RSS dashboard] isomorphic-git proxy failed",
							e,
						);
					}
...
export class CustomXMLParser {
	...
	private parseAtom(doc: Document): ParsedFeed {
		...
		entryElements.forEach((entry, idx) => {
			...
			const content =
				this.getTextContent(entry, "content") || description;
	...
	private parseJSON(jsonString: string): ParsedFeed {
		try {
			...
			if (
				data.version &&
				data.version.startsWith("https://jsonfeed.org/")
			) {
				return {
					title: data.title || "",
	...
	private fallbackParse(xmlString: string): ParsedFeed {
		try {
			...
			if (itemMatches.length === 0) {
				...
				while ((itemMatch = itemStartRegex.exec(cleanedXml)) !== null) {
					...
					const contentStartIndex =
						itemStartIndex + itemStartTag.length;

					// Find where this item ends by looking for next item, channel close, or rss close
					const remainingText =
						cleanedXml.substring(contentStartIndex);
					const nextItemMatch = remainingText.match(/<item[^>]*>/i);
					const channelCloseMatch =
						remainingText.match(/<\/channel>/i);
					...
					if (
						channelCloseMatch &&
						channelCloseMatch.index !== undefined
					) {
			...
			if (itemMatches.length === 0) {
				...
				while (
					(itemMatch = aggressiveItemRegex.exec(xmlString)) !== null
				) {
					itemMatches.push(itemMatch);
				}
			}
			...
			itemMatches.forEach((itemMatch, index) => {
				...
				const itemTitleMatch = itemXml.match(
					/<title[^>]*>([^<]+)<\/title>/i,
				);
				if (!itemTitleMatch) {
					return;
				...
				const itemLinkMatch = itemXml.match(
					/<link[^>]*>([^<]+)<\/link>/i,
				);
				...
				const itemGuidMatch = itemXml.match(
					/<guid[^>]*>([^<]+)<\/guid>/i,
				);
				...
				const startPage = startPageMatch
					...
					: "";
				const endPageMatch = itemXml.match(
					/<endPage[^>]*>([^<]+)<\/endPage>/i,
				);
				...
				const fileSize = fileSizeMatch
					...
					: "";
				const authorsMatch = itemXml.match(
					/<authors[^>]*>([^<]+)<\/authors>/i,
				);
	...
	private extractRssContent(xmlString: string): string {
		...
		if (rssMatch) {
			...
			if (channelMatch) {
				...
				const itemMatches = xmlString.match(
					/<item[^>]*>[\s\S]*?<\/item>/gi,
				);
...
export class FeedParser {
		...
		try {
			...
			content = content.replace(
				...
				(match: string, attributes: string, src: string) => {
					...
					const absoluteSrc = this.convertToAbsoluteUrl(
						decodedSrc,
						baseUrl,
					);
				...
				(match: string, attributes: string, srcset: string) => {
					const processedSrcset = srcset
						...
						.map((part: string) => {
							...
							const urlMatch =
								trimmedPart.match(/^([^\s]+)(\s+\d+w)?$/);
							if (urlMatch) {
								const url = urlMatch[1];
								const sizeDescriptor = urlMatch[2] || "";

								const decodedUrl =
									this.parser.decodeHtmlEntities(url);
			...
			content = content.replace(
				...
				(match: string, attributes: string, href: string) => {
					...
					const absoluteHref = this.convertToAbsoluteUrl(
						decodedHref,
						baseUrl,
					);
	...
	private extractPodcastCoverImage(
		...
		if (item.image?.url) {
			const itemImage = this.convertToAbsoluteUrl(
				item.image.url,
				baseUrl,
			);
			if (itemImage) {
		...
		if (feedImage) {
			...
			if (feedImageUrl) {
				const convertedUrl = this.convertToAbsoluteUrl(
					feedImageUrl,
					baseUrl,
				);
				if (convertedUrl) {
	...
	async parseFeed(
		...
		parsed.items.forEach((item: ParsedItem) => {
			...
			if (existingItem) {
				...
				const updatedItem: FeedItem = {
					...
					content: this.convertRelativeUrlsInContent(
						item.content || "",
						url,
					),
					...
					explicit:
						...
						existingItem.explicit,
					category: item.itunes?.category || existingItem.category,
					episodeType:
						item.itunes?.episodeType || existingItem.episodeType,
				...
				let coverImage = "";
				if (isPodcast) {
					coverImage = this.extractPodcastCoverImage(
						item,
						parsed.image,
						url,
					);
		...
		Object.entries(coverImageCounts).forEach(([imgUrl, count]) => {
			if (
				imgUrl &&
				(imgUrl === feedLogoUrl ||
					feedLogoCandidates.includes(imgUrl)) &&
		...
		if (processedFeed.mediaType === "video" && !existingFeed?.folder) {
			...
			processedFeed.mediaType === "podcast" &&
			!existingFeed?.folder
		) {
