...
export class OpmlManager {
    ...
    const processOutlines = (
      ...
      for (let i = 0; i < outlines.length; i++) {
        ...
        if (!type && outline.hasChildNodes()) {
          ...
          const category =
            outline.getAttribute("category") || currentPath || "Uncategorized";
  ...
  static parseOpmlMetadata(opmlContent: string): {
    feeds: FeedMetadata[];
    folders: Folder[];
  } {
    const parser = new DOMParser();
  ...
  static importOpml(
    ...
    try {
      const { feeds: newFeeds, folders: newFolders } =
        this.parseOpml(opmlContent);
