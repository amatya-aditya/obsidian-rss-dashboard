...
export class EditFeedModal extends Modal {
	...
	onOpen() {
						...
						try {
							...
							if (platform) {
								...
								if (refs.statusDiv)
									refs.statusDiv.textContent = status;
								const resolvedUrl =
									await resolvePodcastPlatformUrl(url);
								if (!resolvedUrl) {
									throw new Error(
										"Could not resolve podcast feed URL",
									);
								}
								feedUrl = resolvedUrl;
								url = resolvedUrl;
					...
					if (value === "custom") {
						if (!autoDeleteCustomInput) {
							autoDeleteCustomInput =
								autoDeleteSetting.controlEl.createEl("input", {
									type: "number",
							...
							autoDeleteCustomInput.value =
								autoDeleteDuration > 0
									? autoDeleteDuration.toString()
									: "";
							autoDeleteCustomInput.addEventListener(
								"change",
								(evt: Event) => {
									const target =
		...
		maxItemsSetting.addDropdown((dropdown) => {
			dropdown
				...
				.setValue(
					maxItemsLimit === 0
						? "0"
						: [10, 25, 50, 100, 200, 500, 1000].includes(
									maxItemsLimit,
							  )
				...
				.onChange((value) => {
					if (value === "custom") {
						if (!maxItemsCustomInput) {
							maxItemsCustomInput =
								maxItemsSetting.controlEl.createEl("input", {
									type: "number",
									placeholder: "Enter number",
									cls: "rss-custom-input",
								});
							maxItemsCustomInput.min = "1";
							maxItemsCustomInput.addEventListener(
								"change",
								(evt: Event) => {
									const target =
										evt.target as HTMLInputElement;
				...
				.onChange((value) => {
					if (value === "custom") {
						if (!scanIntervalCustomInput) {
							scanIntervalCustomInput =
								scanIntervalSetting.controlEl.createEl(
									"input",
									{
										type: "number",
		...
		new Setting(contentEl)
			...
			.setDesc(
				"Select a template to use when saving articles from this feed",
			)
		...
		saveBtn.onclick = () => {
			void (async () => {
				...
				const newMaxItemsLimit =
					maxItemsLimit || this.plugin.settings.maxItems;
				...
				if (
					newMaxItemsLimit > 0 &&
					this.feed.items.length > newMaxItemsLimit
				) {
					...
					this.feed.items = this.feed.items.slice(
						0,
						newMaxItemsLimit,
					);
					new Notice(
