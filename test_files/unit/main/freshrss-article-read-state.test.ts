...
/**
 * Covers the single article-facet mutation boundary (main.ts
 * `commitArticleReadState`) that every read/unread entry point routes
 * through: FreshRSS-linked articles durably queue a pending sidecar mutation
 * before the local facet commits, local-only articles are unaffected, and a
...
describe("commitArticleReadState (single article-facet mutation boundary)", () => {
  beforeEach(() => {
    vi.restoreAllMocks();
    vi.spyOn(obsidian, "requireApiVersion").mockReturnValue(true);
  });

  it("commits a local-only article's read state directly, with no sidecar record created", async () => {
    const { plugin } = createPlugin();
    plugin.settings.feeds = [makeFeed()];

    const result = await plugin.commitArticleReadState([
  ...
  it("leaves a local-only article's read state untouched when FreshRSS is connected but this article has no binding", async () => {
  ...
  it("durably captures a pending facet mutation before committing a FreshRSS-linked article's read state", async () => {
    const { plugin, app } = createFreshRssReadyPlugin();
    plugin.settings.feeds = [makeFeed()];
  ...
  it("replaces the previous pending mutation for the same article with a new operation ID rather than stacking history", async () => {
    const { plugin, app } = createFreshRssReadyPlugin();
    plugin.settings.feeds = [makeFeed()];
    await activateSidecar(app);
    ...
    await plugin.commitArticleReadState([
      { articleGuid: "guid-1", feedUrl: "https://example.test/feed.xml", desiredRead: false },
    ]);
    ...
    expect(plugin.settings.feeds[0].items[0].read).toBe(false);
  });

  it("does not commit the local read state and reports an actionable error when the sidecar write fails", async () => {
    const { plugin, app } = createFreshRssReadyPlugin();
    plugin.settings.feeds = [makeFeed()];
    await activateSidecar(app);
    ...
    // The local facet must remain uncommitted: still unread, and saveSettings
    // was never called for this rejected change.
    expect(plugin.settings.feeds[0].items[0].read).toBe(false);
  ...
  it("batches multiple articles into one sidecar read/write under one lease acquisition", async () => {
    ...
    const result = await plugin.commitArticleReadState([
      { articleGuid: "guid-1", feedUrl: "https://example.test/feed.xml", desiredRead: true },
      { articleGuid: "guid-2", feedUrl: "https://example.test/feed.xml", desiredRead: true },
    ]);

    expect(result).toEqual({ committed: true });
    expect(plugin.settings.feeds[0].items[0].read).toBe(true);
    expect(plugin.settings.feeds[0].items[1].read).toBe(true);
    expect(plugin.saveSettings).toHaveBeenCalledTimes(1);
