...
## Post-2.6.0 remediation status

Remediation is in progress following the post-2.6.0 community plugin scorecard review (44 findings total from automated scans):

1. **`document.createElement` -> Obsidian createEl helpers (34 findings)**: Staged across test and UI files.
2. **`mjx-container` unknown type selector (6 findings)**: Resolved by replacing raw `mjx-container` type selectors in `src/styles/articles.css` and `src/styles/reader.css` with attribute and class selectors (`[class*="mjx-container"]`, `.MathJax`).
3. **Unnecessary assertion in `src/utils/settings-loader.ts:367` (1 finding)**: Resolved by typing `migrateDefaultFilterToDashboardMultiFilters` parameters to accept `DisplaySettings` directly and removing the double cast.
4. **Declarative settings API `getSettingDefinitions()` suggestion (1 finding)**: Evaluated for `PluginSettingTab` in `src/settings/settings-tab.ts`. Obsidian 1.13.0 declarative settings search suggestion; standard imperative `display()` tab implementation remains compliant with `obsidian.d.ts` baseline (1.8.7).
5. **Build verification (1 finding)**: Local `npm run build` completes cleanly (CSS scoping, platform compatibility, zero-`!important`, commit message linting, ESLint with zero warnings, TypeScript compilation, and esbuild production bundling).

### Assessment and disposition table

| Priority                              | Captured finding                                                           | Current disposition                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Follow-up scope and validation                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ------------------------------------- | -------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Done locally; awaiting release/rescan | 34 `document.createElement` occurrences                                    | Remediated: migrated across test fixtures and UI stubs to Obsidian's standard DOM helpers (`createEl`, `createDiv`, `createSpan`). Staged for next release.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Unit tests (192 files, 1734 tests) and ESLint pass cleanly. Verify in next release build.                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Done locally; awaiting release/rescan | 6 `mjx-container` unknown type selector warnings                           | Remediated in `src/styles/articles.css` and `src/styles/reader.css`: replaced raw custom-element type selector `mjx-container` with attribute/class selectors `[class*="mjx-container"]` and `.MathJax`. Preserves normal line-height rules for MathJax formulas while eliminating unknown type selector warnings.                                                                                                                                                                                                                                                                                                                                                                                                                                      | `npm run check:css-scope`, `npm run check:important`, and full build pass cleanly.                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Done locally; awaiting release/rescan | Unnecessary assertion in `src/utils/settings-loader.ts:367`               | Remediated in `src/utils/settings-loader.ts` and `src/utils/settings-migration.ts`: updated parameter type of `migrateDefaultFilterToDashboardMultiFilters` to accept `DisplaySettings | Record<string, unknown>` and `{ statusFilters?: unknown[]; tagFilters?: unknown[]; logic?: string } | Record<string, unknown>`, eliminating unnecessary double casting (`as unknown as Record<string, unknown>`).                                                                                                                                                                                                                                                                                                                                                                                                                 | Strict ESLint, typecheck (`tsc --noEmit --skipLibCheck`), and settings-loader vitest suites pass.                                                                                                                                                                                                                                                                                                                                                                                                                    |
