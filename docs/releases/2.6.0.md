...
> **Draft release notes.** This summary is prepared from `CHANGELOG.md` for
> the 2.6.0 release and remains subject to final validation and publication.
...
```text
RSS Dashboard 2.6.0 is on the way! This release focuses on making bigger feed
libraries smoother and easier to manage:

• Per-feed refresh schedules, clearer refresh status, failed-feed retries, and
  a Stop action for global refreshes/imports
• Optional local image caching plus independent Card and Feed preview controls
• A paged podcast playlist built for large feeds
• Better Reader math support, Windows-1251 feed decoding, saved-template
  selection, and a batch of Dashboard, mobile, and saving fixes

For the complete list of improvements and fixes, please see the changelog and
the full 2.6.0 release notes: <CHANGELOG LINK> | <RELEASE NOTES LINK>
```

Replace the two placeholders with the published GitHub links when the release
is announced.
