...
- 234-05 (idempotent re-import) is expected to build on this: the label→tag shape is `candidate.item.tags?: Tag[]`, already directly mergeable with `mergeTagArrays` in `src/utils/tag-utils.ts`.
