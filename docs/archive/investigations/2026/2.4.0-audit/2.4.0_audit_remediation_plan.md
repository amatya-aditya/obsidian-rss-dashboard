...
2. **Ignore scratch files** — add `.tmp-*` to `ignores`; delete the historical `.tmp-eslint-directive.ts` scratch file (it previously broke `npm run lint:dev`).
...
**Goal:** Every remaining declaration is either **removed** or **justified** with inline `/* audit-ok: … */` per [`CONTRIBUTING.MD`](CONTRIBUTING.MD) and [2.3.0 CSS templates](../2.3.0-audit/CSS%20Refactor%20Prompt%20Template.md).
...
3. Update checkboxes in [the 2.4.0 audit record](2.4.0%20audit.md)
