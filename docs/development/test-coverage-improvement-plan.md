...
| Failing Tests                | 0 (0%)                                     |
| Line Coverage (global)       | 35.19%                                     |
| Branch Coverage (global)     | 29.98%                                     |
| Function Coverage (global)   | 29.17%                                     |
| Coverage Thresholds (global) | lines: 35 \| branches: 29 \| functions: 29 |
...
Current thresholds (2026-03-29): `lines: 35`, `branches: 29`, `functions: 29`.
...
- Current test run results (2026-03-29): 80 passing test files, 590 passing tests, 0 failing
- Current global snapshot (2026-03-29, includes `main.ts`): Lines 35.19% | Branches 29.98% | Functions 29.17%
- `main.ts` is included in coverage collection as of P2-6 (`vitest.config.mjs` `coverage.include` includes `main.ts`).

## Post P2-6: Next Focus (P3)

### Priority: Modals + Settings (highest ROI)

**Why:** Modals are currently the biggest gap (3.89% line coverage measured) and represent high-risk user flows (feed management, OPML import/export, navigation). Settings have low branch coverage and are a frequent source of regressions.

**Constraints:** No new dependencies for this push (Vitest + jsdom + existing stubs).

**Immediate deliverables (P3-1):**

- Add fixtures: `test_files/fixtures/opml/*` (at least `single-feed.opml`, `nested-folders.opml`, `invalid.opml`)
...
- Expand Obsidian stubs only as needed to support modal/settings behavior (keep changes minimal and documented).

### Top ROI targets (today’s measured line coverage)

- `src/modals/feed-manager/feed-manager-modal.ts` (0.00%)
- `src/modals/import-opml-modal.ts` (0.20%)
- `src/components/sidebar.ts` (3.94%)
- `src/views/discover-view.ts` (10.24%)
...
### Recommended: Post P2-6 ratchet

P2-6 (including `main.ts` in coverage collection) is complete. Next, add targeted tests (especially in `main.ts`) to recover the small global drop and meet the ratchet rule (`threshold + 0.75%`) so thresholds can be bumped again.

### Alternative: Continue with existing phase handoffs

- P2-3: Video player tests (`docs/development/p2-3-handoff.md`)
- Continue raising coverage in low-coverage components/modals before expanding `coverage.include`
